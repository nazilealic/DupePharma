# REST API Görev Dağılımı

**REST API Adresi:** [api.yazmuh.com](https://dupepharma.onrender.com)

Bu dokümanda, proje ekibindeki her üyenin geliştirmekten sorumlu olduğu REST API metotları listelenmektedir.

---

## Grup Üyelerinin REST API Metotları

1. [Nazile Alıç'ın REST API Metotları](nazile-alic/Nazile-Alic-Rest-API-Gorevleri.md)
2. [Şadiye Berra Özelgül'ün REST API Metotları](Sadiye-Berra-Ozelgul/Sadiye-Berra-Ozelgul-Rest-API-Gorevleri.md)
3. [Menekşe Nazik'in REST API Metotları](menekse-nazik/menekse-nazik-Rest-API-gorevleri.md)
4. [Bahar Balım'ın REST API Metotları](Bahar-Balim/Bahar-Balim-Rest-API-Gorevleri.md)
