# Gereksinim Analizi

Tüm gereksinimlerinizi çıkardıktan sonra beraber tartışıyoruz ve son gereksinimlerin isimlerini hangi API metoduna karşılık geleceğini ve kısa açıklamalarını buraya numaralı bir şekilde yazıyorsunuz. Daha sonra aşağıya herkes kendi gereksinimiyle ilgili sayfayı oluşturmalı ve kendi sayfasında kendine ait gereksinimleri numaralı bir şekilde listeleyerek her bir gereksinimin açıklamalarını yazmalı. Toplamda grup üyesi sayısı kadar sayfa oluşturulmalı. Her grup üyesine eşit sayıda gereksinim atanmalı.

## Gereksinim Sayıları (En Az)

- **1 Kişi:** 10 gereksinim
- **2 Kişi:** 16 gereksinim
- **3 Kişi:** 21 gereksinim
- **4 Kişi:** 24 gereksinim
- **5 Kişi:** 30 gereksinim

## Gereksinimlerde Uyulması Gereken Kurallar

1. **İsimler anlamlı olmalı:** Gereksinim isimleri net ve anlaşılır olmalıdır.
2. **Açıklamalar net olmalı:** Her gereksinimin açıklaması açık ve anlaşılır şekilde yazılmalıdır.
3. **Açıklamalar teknik jargon ve kısaltmalar içermemeli:** Gereksinim açıklamaları herkesin anlayabileceği basit bir dille yazılmalıdır.
4. **Gereksinim isimleri çok uzun olmamalı ve bir eylem bildirmeli:** 
   - İsimler kısa ve öz olmalıdır
   - Bir eylem fiili içermelidir
   - Örnekler: "Kayıt Olma", "Giriş Yapma", "Profil Güncelleme", "Hesap Silme"

# Tüm Gereksinimler 

1- **Ürünlerin listelenmesi** (Nazile Alıç)
2- **Kozmetik ürün için muadil ürünleri listelemek** (Nazile Alıç)
3- **Muadil ürünler için fiyat karşılaştırması** (Nazile Alıç)
4- **Ürün araması yapma** (Nazile Alıç)
5- **Arama geçmişini görüntüle** (Nazile Alıç)
6- **Arama Geçmişini sil** (Nazile Alıç)
7- **Cilt profili oluşturma** (Nazile Alıç)
8- **Cilt profili güncelleme** (Nazile Alıç)
9- **Kategori filtrelemesi** (Şadiye Berra Özelgül)
10- **Fiyat aralığını filtreleme** (Şadiye Berra Özelgül)
11- **Favorilere ekle** (Şadiye Berra Özelgül)
12- **Favorilerden çıkar** (Şadiye Berra Özelgül)
13- **Favorileri listele** (Şadiye Berra Özelgül)
14- **Nöbetçi eczane listesini düzenle** (Şadiye Berra Özelgül)
15- **Listeyi görüntüle** (Şadiye Berra Özelgül)
16- **Ürünlere yorum ekleme** (Menekşe Nazik) 
17- **Yorum silme** (Menekşe Nazik)
18- **Yorum listeleme** (Menekşe Nazik)
19- **Yorum güncelleme** (Menekşe Nazik)
20- **Kullanıcıların ürünlere puan verme** (Menekşe Nazik)
21- **Kullanıcı giriş işlemleri** (Menekşe Nazik)
22- **Kullanıcı çıkış işlemleri** (Menekşe Nazik)
23- **Üye olma** (Bahar Balım)
24- **Şifre yenileme** (Bahar Balım)
25- **Admin paneli üzerinden ürün ekleme** (Bahar Balım)
26- **Admin paneli üzerinden ürün düzenleme** (Bahar Balım)
27- **Admin paneli üzerinden ürün silme** (Bahar Balım)
28- **Yapay zeka ile ürün analizi** (Bahar Balım)
29- **Ürünlerin içerik bilgilerini görüntüleme** (Bahar Balım)

# Gereksinim Dağılımları

1. [Nazile Alıç Gereksinimleri](nazile-alic/nazile-alic-gereksinimler.md)
2. [Şadiye Berra Özelgül Gereksinimleri](Sadiye-Berra-Ozelgul/Sadiye-Berra-Ozelgul-Gereksinimler.md)
3. [Menekşe Nazik Gereksinimleri](menekse-nazik/menekse-nazik-gereksinimler.md)
4. [Bahar Balım Gereksinimleri](bahar-balim/bahar-balim-gereksinimler.md)
